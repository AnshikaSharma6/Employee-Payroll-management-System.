...
class Login:
    ...
    def login_function(self):
        ...
        elif self.txt_pass.get() == "123456" and self.txt_user.get() == "Anshika":
            isAdmin=False
...
#=====================Employee Area=========================================================
           ...
           # messagebox.showinfo("Welcome", f"Welcome {self.txt_user.get()}\nYour Password: {self.txt_pass.get()}", parent=self.root)
            ...
            self.btn_Save=Button(Frame_a,text="Save",font=("times new roman",15),bg="light green",fg="black")
            self.btn_Save.place(x=200,y=605,height=30,width=140)
...
'''
        ...
        def check_connection(self):
            try:
                ...
                cur=con.cursor()
                cur.execute("select * from emps_salary where name=txt_user")
                rows=cur.fetchall()
                print(rows)
            ...
            # print(rows)
                if row!=None:
                   messagebox.showerror("Error","This Employee is already Available in record ,try again with ID",parent=self.root)
                    ...
                    cur.execute("insert into epms_salary values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                    (
            ...
            # print(rows)
                if row==None:
                   messagebox.showerror("Error","This Employee Id is invalid,try again with valid Employee ID",parent=self.root)
                    ...
                    cur.execute("UPDATE `epms_salary` SET `E_ID`=%s,`designation`=%s,`name`=%s,`age`=%s,`gender`=%s,`email`=%s,`hr_location`=%s,`doj`=%s,`dob`=%s,`experience`=%s,`proof_id`=%s,`contact`=%s,`status`=%s,`address`=%s,`basic_salary`=%s,`absent_days`=%s,`medical`=%s,`pf`=%s,`convence`=%s,`net_ salary`=%s,`salary_receipt`=%s WHERE 1",
                    (
    ...
    def calculate(self):
        if  self.var_salary.get()=='' or self.var_absent.get()=='' or self.var_medical=='' or self.var_pf.get()=='' or self.var_convence.get()=='' or self.txt_address.get('1.0',END)=='':
           ...
           # work_day=int(self.var_t_days.get())-int(self.var_absent.get())
            date_of_join = datetime.strptime(str(self.var_doj), "%d-%m-%Y")
            now = datetime.now()
...
Total Present\t\t:{str(self.var_doj)-(net_sal)}
